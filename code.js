$(function () {
  ...
  (function init() {
    ...
    let jsonTestCase = {
      ...
      "message": "ok",
      "data": [{
        "id": "6e9485ae-7d80-4438-803a-c3df575df793",
        "title": "Kesempatan Belajar Seni secara Daring Makin Terbuka",
        "image_url": "https://mejaseni.yk1.s3.gmedia.id/media/v4PuXMcTsGI50Chf22oJ1lHtijGnsIOpqUDHwk0a.jpg",
        "date": "Selasa, 08/06/2021"
      }],
      "paginate": {
        "total": 3,
        "total_filtered": 3
    ...
    function generate() {
      ...
      let objToDart = (jsonObj, prefix, baseClass) => {
          ...
          if (jsonObj.hasOwnProperty(key)) {
            ...
            if (typeof element === 'object') {
              ...
              if (Array.isArray(element)) {
                ...
                propsLines.push(`  final ${genericString} ${legalKey};\n`);
                fields.push(`${legalKey}: ${legalKey},`);
                fieldModel.push(`  required ${genericString.replaceAll('>', 'Model>')} ${legalKey},\n`);
                ...
                lines.unshift(objToDart(element, className, key));
                propsLines.push(`  final ${subClassName}${shouldNullSafe ? '?' : ''} ${legalKey};\n`);
                fields.push(`${legalKey}: ${legalKey},`);
                fieldModel.push(`  required ${subClassName}Model ${legalKey},\n`);
              ...
              propsLines.push(`  final ${type}${shouldNullSafe ? '?' : ''} ${legalKey};\n`);
              fields.push(`${legalKey}: ${legalKey},`);
              fieldModel.push(`  required ${type} ${legalKey},\n`);
        ...
        constructorLines.push(`  ${fieldModel.join('')}}) \: super(${fields.join('')}) ;`);
        fromJsonLines.push(`  }`);
      ...
      //!json对象转dart Entity
      let objToDartEntity = (jsonObj, prefix, baseClass) => {

        if (Array.isArray(jsonObj)) {
          return objToDartEntity(jsonObj[0], prefix, baseClass);
        }
        ...
        lines.push(`class ${className}Entity extends Equatable{`);
          ...
          if (jsonObj.hasOwnProperty(key)) {
            ...
            if (typeof element === 'object') {
              ...
              if (Array.isArray(element)) {
                ...
                propsLines.push(`  final ${genericString.replaceAll('>', 'Entity>')} ${legalKey};\n`);
                fields.push(`${legalKey},`);
                fromJsonLines.push(fromJsonLinesJoined);
                toJsonLines.push(toJsonLinesJoined);
                if (typeof inner === 'object') {
                  lines.unshift(objToDartEntity(element, className, key));
                }
              } else {

                lines.unshift(objToDartEntity(element, className, key));
                propsLines.push(`  final ${subClassName}Entity${shouldNullSafe ? '?' : ''} ${legalKey};\n`);
                fields.push(`${legalKey},`);
                let typeCheck = shouldEnhanceFaultTolerance ? ` && (json[${jsonKey}] is Map)` : '';
                fromJsonLines.push(`    ${legalKey} = (json[${jsonKey}] != null${typeCheck}) ? ${subClassName}.fromJson(json[${jsonKey}]) : null;\n`);
                toJsonLines.push(`    if (${legalKey} != null) {\n      data[${jsonKey}] = ${thisData}${legalKey}${shouldNullSafe ? '!' : ''}.toJson();\n    }\n`);
        ...
        lines.push(`  @override`);
        lines.push(`  List<Object> get props => [${fields.join('')}];`);
      ...
      let dartCodeEntity = `${objToDartEntity(jsonObj, rootClass, "")}`;
      let dartCodeModel = `${objToDart(jsonObj, rootClass, "")}`;


      resultDartCode = dartCodeEntity;
      resultDartCodeModel = dartCodeModel;

      let highlightDartCode = hljs.highlight('dart', dartCodeEntity);
      let highlightDartCodeModel = hljs.highlight('dart', resultDartCodeModel);
      $('#dartCode').html(highlightDartCode.value + `\n` + highlightDartCodeModel.value);
